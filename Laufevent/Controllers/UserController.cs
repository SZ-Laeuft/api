...
namespace Laufevent.Controllers
{
    ...
    public class UserController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetUserByUID([FromQuery] decimal uid)
        {
            try
            {
                using (var connection = new NpgsqlConnection(ConnectionString.connectionstring))
                        ...
                        using (var reader = await command.ExecuteReaderAsync())
                        {
                            if (await reader.ReadAsync())
                            {
                                var user = new
                                {
                                    ...
                                    Uid = reader["uid"] is DBNull ? 0L : Convert.ToDecimal(reader["uid"]),
...
[SwaggerOperation(
    ...
    Description = "Updates user details such as first name, last name, school class, and organization."
)]
...
public async Task<IActionResult> UpdateUserById(decimal uid, [FromBody] UpdateUserModel userInfo)
{
    if (userInfo == null)
    {
        return BadRequest("The userInfo field is required.");
    }
    ...
    try
    {
        using (var connection = new NpgsqlConnection(ConnectionString.connectionstring))
        {
            ...
            using (var command = new NpgsqlCommand(query, connection))
            {
                ...
                if (rowsAffected > 0)
                {
                    return Ok($"User with UID {uid} successfully updated.");
                }
                else
                {
                    return NotFound($"User with UID {uid} not found.");
    ...
    catch (Exception ex)
    {
        ...
        [SwaggerResponse(500, "Internal Server Error - Database issue or unexpected error.")]
        public async Task<IActionResult> DeleteUserByUid(decimal Uid)
        {
            try
            {
                using (var connection = new NpgsqlConnection(ConnectionString.connectionstring))
